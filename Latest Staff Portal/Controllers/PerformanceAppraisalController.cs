    ...
    public class PerformanceAppraisalController : Controller
    {
        ...
        public PartialViewResult HRAppraisalsListPartialView(string Appraisal_Stage, string status)
        {
            try
            {
                ...
                // Build OData query
                string page;
                if (Appraisal_Stage is "Target Setting" or "Target Approval")
                {
                    page = $"Hr360AppraisalCard?$filter=Appraisal_Stage eq '{Appraisal_Stage}'&$format=json";
                }
                else if (Appraisal_Stage is "Approved" or "Closed" or "HR")
                {
                ...
                var json = new StreamReader(httpResponse.GetResponseStream()).ReadToEnd();
                var details = JObject.Parse(json)["value"]?.ToObject<List<AppraisalsList>>() ?? new List<AppraisalsList>();

                // Filter per record based on role
                var filteredList = details.Where(x =>
                {
                    if (x.Supervisor == userId) // Supervisor rules
                    {
                        return Appraisal_Stage switch
                        {
                            "Appraisee Reviewed" => x.Status == Appraisal_Stage,
                            "Immediate Supervisor Reviewed" => x.Status == Appraisal_Stage,
                            "Appeal Stage" => x.Status == Appraisal_Stage,
                            "Approved" or "Closed" or "HR" =>
                                x.Appraisal_Stage == "End Year Evalauation" && x.Status == "Approved/Closed/HR",
                            "Target Approval" => x.Appraisal_Stage == Appraisal_Stage,
                            _ => x.User_ID == userId // Supervisor also creator
                        };
                    }
                    else // Employee rules
                    {
                        return Appraisal_Stage switch
                        {
                            "Appraisee Reviewed" => x.Status == Appraisal_Stage && x.User_ID == userId,
                            "Immediate Supervisor Reviewed" => x.Status == Appraisal_Stage && x.User_ID == userId,
                            "Appeal Stage" => x.Status == Appraisal_Stage && x.User_ID == userId,
                            "Approved" or "Closed" or "HR" =>
                                x.Appraisal_Stage == "End Year Evalauation" && x.Status == "Approved/Closed/HR" && x.User_ID == userId,
                            _ => x.User_ID == userId
                        };
                    }
                });
                ...
                return PartialView(
                    "~/Views/PerformanceAppraisal/PartialViews/HRAppraisalsListPartialView.cshtml",
                    filteredList.OrderByDescending(x => x.Appraisal_No)
                );
            }
            catch (Exception ex)
            {
                var erroMsg = new Error { Message = ex.Message.Replace("'", "") };
        ...
        public PartialViewResult OtherDutiesLines(string AppraisalNo, string Status, string Appraisal_Stage, string Appraisal_Period)
        {
            try
            {
                var employeeView = Session["EmployeeData"] as EmployeeView;
                ...
                var otherDutiesLines = new List<OtherDuties>();
                var pageLine = $"ApiIOService?$filter=Appraisal_No eq '{AppraisalNo}'  and Appraisal_Period eq '{Appraisal_Period}'&$format=json";
                var httpResponseLine = Credentials.GetOdataData(pageLine);
                ...
                using (var streamReader = new StreamReader(httpResponseLine.GetResponseStream()))
                {
                    ...
                    foreach (JObject config in details["value"])
                    {
                        var respLine = new OtherDuties
                        {
                            Appraisal_No = (string)config["Appraisal_No"],
                            Line_No = (int)config["Line_No"],
                            Employee_No = (string)config["Employee_No"],
                            Appraisal_Period = (string)config["Appraisal_Period"],
                            Duties = (string)config["Duties"],

                        };

                        otherDutiesLines.Add(respLine);
                    }
                }
                ViewBag.Status = Status;
                ViewBag.Appraisal_Stage = Appraisal_Stage;
                return PartialView("~/Views/PerformanceAppraisal/PartialViews/OtherDutiesLines.cshtml", otherDutiesLines);
            }
            catch (Exception ex)
            {
                var erroMsg = new Error
                ...
                using (var streamReader = new StreamReader(httpResponseLine.GetResponseStream()))
                {
                    ...
                    foreach (JObject config in details["value"])
                    {
                        var goalsLine = new PersonalGoalsObjectives
                        {
                            ...
                            Self_Rating = (decimal)config["Self_Rating"],
                            Moderated_Score = (decimal)config["Moderated_Score"],
                            Employee_Comments = (string)config["Employee_Comments"],
                            Supervisor_Rating = (decimal)config["Supervisor_Rating"],
        ...
        public ActionResult NewKPI(string Appraisal_No, string Line_No, string Perfomance_Goals_and_Targets, string totalWeight)
        {
            try
            {
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public JsonResult SubmitKPI(string Appraisal_No, string Line_No, string Perfomance_Goals_and_Targets, decimal Weight, string KPI)
        {
            try
            {
                var UserID = Session["UserID"].ToString();
                var StaffNo = Session["Username"].ToString();
                var employeeView = Session["EmployeeData"] as EmployeeView;
                int res = Credentials.ObjNav.InsertKpiRecord(int.Parse(Line_No), Appraisal_No, KPI, Weight);
        ...
        public JsonResult SubmitObjective(string Appraisal_No, string Perfomance_Goals_and_Targets, string Weight, string Key_Perfomance_Indicator)
        {
            try
            {
                ...
                res = Credentials.ObjNav.AddHrAppraisalLines(Appraisal_No, Perfomance_Goals_and_Targets, "");
                return Json(new { message = "Record Added successfully", success = true }, JsonRequestBehavior.AllowGet);
        ...
        public JsonResult UpdateObjectiveLine(PersonalGoalsObjectives data)
        {
            try
            {
                ...
                res = Credentials.ObjNav.ModifyHrAppraisalLines(data.Appraisal_No, data.Line_No, data.Perfomance_Goals_and_Targets, 0, data.Weight, data.Key_Perfomance_Indicator, data.Self_Rating, data.Employee_Comments, data.Supervisor_Rating, data.Supervisor_Comments, data.Moderated_Score);
        ...
        public ActionResult NewOtherDuty(string docNo, string Appraisal_Period)
        {
            try
            {
                var OtherDutyLine = new OtherDuties();

                OtherDutyLine.Appraisal_No = docNo;
                OtherDutyLine.Appraisal_Period = Appraisal_Period;
                ...
                return PartialView("~/Views/PerformanceAppraisal/PartialViews/NewOtherDuty.cshtml", OtherDutyLine);
            }
            catch (Exception ex)
            {
                var erroMsg = new Error();
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public JsonResult SubmitOtherDuty(string Appraisal_No, string Appraisal_Period, string Duties)
        {
            try
            {
                ...
                var employeeView = Session["EmployeeData"] as EmployeeView;
                string res = "";
                Credentials.ObjNav.InsertOtherDuty(StaffNo, Appraisal_Period, Appraisal_No, Duties);
                return Json(new { message = "Record Added successfully", success = true }, JsonRequestBehavior.AllowGet);
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public JsonResult UpdateOtherDutyLine(PersonalGoalsObjectives data)
        {
            try
            {
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public JsonResult DeleteOtherDutyLine(PersonalGoalsObjectives data)
        {
            try
            {
                var UserID = Session["UserID"].ToString();
                ...
                Credentials.ObjNav.DeleteOtherDuty(data.Appraisal_No, data.Line_No);
        ...
        public JsonResult UpdateEvaluationLine(PersonalGoalsObjectives data)
        {
            try
            {
                ...
                res = Credentials.ObjNav.SupervisorsRating(data.Appraisal_No, data.Line_No, (int)data.Supervisor_Rating, data.Supervisor_Comments);
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public JsonResult SendForModeration(string Appraisal_No)
        {
            try
            {
                ...
                var StaffNo = Session["Username"].ToString();
                var employeeView = Session["EmployeeData"] as EmployeeView;
                Credentials.ObjNav.SendforModeration(Appraisal_No);

                return Json(new { message = "Appraisal successfully sent for moderation", success = true }, JsonRequestBehavior.AllowGet);
